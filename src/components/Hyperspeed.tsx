...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = (options.colors.sticks as any).map((c: number) => new THREE.Color(c));
    } else {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, className = '' }) => {
  ...
  return <div className={`w-full h-full ${className}`} ref={hyperspeed}></div>;
};
