...
// Email templates
const LOGO_URL = 'https://cqbwkhhefnurmddwazsk.supabase.co/storage/v1/object/public/email-assets/logo-rawinet.png';
const EMAIL_HEADER_BG = 'linear-gradient(135deg, hsl(220, 50%, 15%) 0%, hsl(220, 45%, 25%) 50%, hsl(195, 60%, 30%) 100%)';
const PRIMARY_COLOR = 'hsl(195, 85%, 45%)';
  ...
  return `
    <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #ffffff;">
      <div style="background: ${EMAIL_HEADER_BG}; padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
        ...
        <p style="color: rgba(255,255,255,0.8); margin: 0; font-size: 14px;">Panel Klienta</p>
      </div>
      <div style="background: #ffffff; padding: 30px; border: 1px solid hsl(210, 20%, 88%); border-top: none; border-radius: 0 0 12px 12px;">
        <h2 style="color: ${FOREGROUND_COLOR}; margin-top: 0;">Twoje dane logowania</h2>
        <p style="color: ${MUTED_COLOR};">Twoje hasło do Panelu Klienta RAWI-NET zostało wygenerowane:</p>
        <div style="background: hsl(210, 20%, 96%); border: 2px dashed hsl(210, 20%, 82%); border-radius: 12px; padding: 20px; text-align: center; margin: 20px 0;">
          <p style="font-size: 24px; font-weight: bold; color: ${FOREGROUND_COLOR}; letter-spacing: 2px; margin: 0;">${password}</p>
        </div>
